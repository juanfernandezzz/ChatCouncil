...
Ocho sistemas leyeron un mismo conjunto de respuestas y registraron lo que
observaron. La tabla de abajo reune todos esos registros. Tu tarea es leer
esa tabla y escribir un informe de lo que muestra.
...
Cada linea tiene seis campos: el identificador del hallazgo, su categoria,
su eje, las respuestas que lo sostienen, su descripcion, y el operador que
lo registro.
...
Solo la primera se puede resolver con evidencia. Las otras tres necesitan
una decision de quien lee el informe.

2. CALIDAD DE LAS CONVERGENCIAS
Para cada convergencia registrada, di si las respuestas que coinciden citan
las mismas fuentes o fuentes distintas. Coincidir citando la misma fuente
unica no es lo mismo que coincidir por caminos independientes.
Si la tabla no trae informacion de fuentes para una convergencia, dilo.

3. LO QUE NADIE DIJO
Reune las ausencias registradas y di que partes de la pregunta original
quedaron sin responder por ninguna de las respuestas.

4. QUE HARIA FALTA
Para cada divergencia, di que dato, que fuente o que decision permitiria
resolverla. Si una divergencia no se puede resolver con mas informacion,
dilo tambien.
...
REGLA OBLIGATORIA: cada parrafo que escribas tiene que contener al menos una
referencia a un hallazgo de la tabla, con su identificador entre corchetes.
Por ejemplo: [H12]. Si un parrafo se apoya en varios hallazgos, ponlos
todos: [H12] [H31].

No puedes afirmar nada que no este en la tabla. Si la tabla no alcanza para
responder algo de las cuatro secciones, escribelo en un parrafo que empiece
con LA TABLA NO ALCANZA y explica que falta. Ese parrafo es el unico que
puede ir sin referencias.

TRES COSAS QUE NO TIENES QUE HACER

No busques informacion. No uses lo que sabes del tema por fuera de la tabla.
Trabaja solo con lo que la tabla dice.

No decidas quien tiene razon. Tu informe describe como se relacionan los
registros, no cual es correcto.

No trates de averiguar que sistema produjo cada respuesta. Los
